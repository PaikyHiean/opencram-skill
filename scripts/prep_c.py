"""
prep_c.py — 为 C 速查卡切出"每章输入包"。

把每一章涉及的源文件 content.json 汇成精简包 work/distilled/c/in_<k>.json，
供子 agent 只读这一章、抽取规则/流程/对比表/公式，产出 out_<k>.json。

与 L2 包相比，C 包**含备注**（规则/流程条件常藏在备注里），也保留表格。
...
def main() -> int:
    ...
    for k, (ch_title, files) in enumerate(chapter_blocks(chapters_doc), start=1):
        ...
        for f in files:
            ...
            for s in content["slides"]:
                ...
                slides.append({
                    ...
                    "tables": s.get("tables", []),
                    "notes": s.get("notes", ""),   # 含备注：规则/流程条件常藏于此
                })
        bundle = {
            "chapter_index": k,
        ...
        index.append({
            ...
            "slides": len(slides),
            "in": str(in_path),
            "out": str(C_DIR / f"out_{k}.json"),
        })

    C.write_json(C_DIR / "index.json", {"chapters": index})
    print("=== C 速查卡输入包已切好 ===")
    for it in index:
